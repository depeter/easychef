...
namespace EasyChef.Contracts.Shared.RestClients
{
    ...
    public class ProductRestClient : RestClient, IProductRestClient
    {
        ...
        public async Task<HttpResult<ProductDTO>> Post(ProductDTO productDto) {
            var response = await _httpClient.PostAsync(_apiBaseUrl + "/api/Product", new StringContent(JsonConvert.SerializeObject(productDto), Encoding.UTF8, "application/json"));
            return await TransformResponse<ProductDTO>(response);
        }

        public async Task<HttpResult<ProductDTO>> Put(ProductDTO productDto) {
            var response = await _httpClient.PutAsync(_apiBaseUrl + "/api/Product", new StringContent(JsonConvert.SerializeObject(productDto), Encoding.UTF8, "application/json"));
            return await TransformResponse<ProductDTO>(response);
        }
    }
}
